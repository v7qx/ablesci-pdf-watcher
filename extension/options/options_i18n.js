...
(function () {
  ...
  "正常：": "OK: ",
  "DOI 批量下载": "Batch DOI Download",
  "实验工具": "Experimental Tools",
  "导入 DOI 清单": "Import DOI List",
  "支持 JSONL、JSON、CSV 和 TXT；自动识别 DOI，并按出版社分槽下载。始终仅下载，不会求助或上传。": "Supports JSONL, JSON, CSV, and TXT. Detects DOI values and downloads through separate publisher lanes. It never requests assistance or uploads.",
  "支持 JSONL、JSON、CSV、TSV 和 TXT；自动识别常见字段并按出版社分槽下载。始终仅下载，不会求助或上传。": "Supports JSONL, JSON, CSV, TSV, and TXT. Detects common fields and downloads through separate publisher lanes. It never requests assistance or uploads.",
  "支持 JSONL、JSON、CSV 和 TXT；自动识别 DOI，并按原有出版社逻辑串行下载。始终仅下载，不会求助或上传。": "Supports JSONL, JSON, CSV, and TXT. Detects DOI values and downloads serially through the existing publisher workflow. It never requests assistance or uploads.",
  "导入": "Import",
  "批量任务": "Batch Tasks",
  "页面关闭时停止；重新打开设置页后可继续。任务间隔固定为 2 分钟，等待期间可手动跳过。": "Stops when this page closes; reopen Settings to resume. The interval is fixed at 2 minutes and can be skipped manually while waiting.",
  "页面关闭时停止；重新打开设置页后可继续。ScienceDirect、Wiley、其他出版社各用一个槽，槽内间隔固定为 2 分钟，等待期间可手动跳过。": "Stops when this page closes; reopen Settings to resume. ScienceDirect, Wiley, and other publishers use separate lanes, each with a fixed 2-minute interval that can be skipped manually.",
  "开始/继续": "Start / Resume",
  "跳过等待": "Skip Wait",
  "完成当前项后暂停": "Pause After Current",
  "导出结果": "Export Results",
  "清空任务": "Clear Tasks",
  "状态": "Status",
  "尚未导入任务。": "No tasks imported.",
  "PDF 保存到 Downloads/DOIPDF/files/期刊，命名为 record_id_title.pdf；完整结果写入 index，失败和无权限记录另写入 issues。": "PDFs are saved under Downloads/DOIPDF/files/journal as record_id_title.pdf. Full results go to index; failures and access issues go to issues.",
  ...
  "上传前核对 PDF 标题": "Validate PDF Title Before Upload",
  "默认关闭。开启后才调用本地 Poppler，标题无法确认或不匹配时仅下载、不上传。": "Off by default. When enabled, local Poppler is invoked; unverifiable or mismatched titles are downloaded only and not uploaded.",
  ...
  "多出版社并发值守": "Multi-publisher concurrent watcher",
  "默认关闭。启用后，仅处理下方已勾选并生成到列表 URL 的出版社；Elsevier 使用 1 个独立槽，其他出版社共享 2 个槽。": "Disabled by default. When enabled, only publishers selected below and written to the list URLs are processed; Elsevier uses one dedicated slot and other publishers share two slots.",
